...
| 37 | 2026-06-07 | Validation DTOs manquante sur 3 endpoints critiques — (1) POST /sessions/start : instanceCount non validé → BigInt(NaN) = TypeError 500 ; ajout StartSessionDto (@IsInt @Min(1) @Max(20) @Type(Number), @IsString @IsNotEmpty sur platform/streamUrl, @Length(2,2) sur preferProxyCountry), (2) PATCH /admin/users/:id/bandwidth : bytesDelta non validé → delta 0 ou non-number silencieux ; ajout AdjustBandwidthDto (@IsNumber @IsNotIn([0])), (3) POST /billing/checkout : successUrl/cancelUrl non validés → erreur Stripe propagée en 500 ; ajout CreateCheckoutDto (@IsUrl) |
