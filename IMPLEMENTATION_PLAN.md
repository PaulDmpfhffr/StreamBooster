...
| 63 | 2026-06-07 | 2 bugs TypeScript : (1) sessionManager.ts — heartbeatTimer typé NodeJS.Timer incompatible avec clearInterval (attend NodeJS.Timeout). Fix : ReturnType<typeof setInterval>. (2) Layout.tsx — import Settings inutilisé (noUnusedLocals:true). Fix : import retiré |
| 64 | 2026-06-07 | Pas de CI. Création .github/workflows/ci.yml : 3 jobs parallèles — test API (jest --forceExit) + typecheck API + typecheck Web + typecheck Desktop. Déclenché sur push/PR vers main |
